...
export default function BountiesPage() {
  ...
  const fetchBounties = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/bounties?status=${status}`);
      if (response.ok) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-bold text-[#F8FAFC] mb-2">Bounties</h1>
          <p className="text-[#94A3B8]">Browse and create bounties</p>
        </div>
        <Button
          asChild
          className="bg-[#00FFB3] text-[#0F172A] hover:bg-[#00E6A3]"
        >
          <Link href="/app/bounties/create">Create Bounty</Link>
        </Button>
      </div>
          ...
          {loading ? (
            ...
            <Card className="bg-[#1E293B] border-[#334155] p-12 text-center">
              ...
              <Button
                ...
                className="bg-[#00FFB3] text-[#0F172A] hover:bg-[#00E6A3]"
              >
                <Link href="/app/bounties/create">Create First Bounty</Link>
              </Button>
            </Card>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {bounties.map((bounty) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-[#334155]">
                    ...
                    >
                      <Link href={`/app/bounty/${bounty.id}`}>View</Link>
                    </Button>
                  </div>
                </Card>
              ))}
